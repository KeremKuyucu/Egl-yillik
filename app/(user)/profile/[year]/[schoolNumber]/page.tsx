...
function LockedMemoryCard({
    ...
    return (
        <div className="group relative overflow-hidden rounded-2xl border border-white/60 dark:border-slate-700/50 bg-white/80 dark:bg-slate-900/80 shadow-lg hover:shadow-xl backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5">
            {/* Sol gradient şerit - aynı unlocked gibi */}
            ...
            <div className="p-6">
                ...
                <div className="flex items-center gap-4 mb-5">
                    <div className="relative group/avatar">
                        ...
                        <div className={`relative h-14 w-14 rounded-xl flex items-center justify-center text-base font-bold shadow-lg ${getColorFromName(author.first_name)} group-hover/avatar:scale-105 transition-transform duration-300`}>
                            {initials}
                        </div>
                    </div>
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-2">
                            <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-md text-xs font-medium">
                                {author.class}
                            </span>
                            <span className="text-slate-400">#{author.school_number}</span>
                        </p>
                    </div>
                ...
                <div className="relative pl-5 border-l-2 border-gradient-to-b from-indigo-300 to-purple-300 dark:from-indigo-700 dark:to-purple-700 ml-2">
                    ...
                    {/* Blur overlay sadece mesaj üzerinde */}
...
export default async function ProfilePage({ params }: ProfilePageProps) {
    ...
    const targetYear = year && !isNaN(Number(year)) ? Number(year) : null
    ...
    const totalVotesForMemories = allCategoriesWithVotes.reduce(
                            ...
                            {!isOwnProfile && (
                                <Link href={`/new?recipientId=${profile.id}`} prefetch={false}>
                                    <Button className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white shadow-xl shadow-purple-500/25 hover:shadow-purple-500/40 hover:scale-105 transition-all duration-300 px-6">
                                        <PenLine className="mr-2 h-4 w-4" suppressHydrationWarning />
                                        Anı Yaz
                                    </Button>
                                </Link>
                            )}
                                ...
                                <div className="relative overflow-hidden rounded-[1.5rem] border border-white/60 dark:border-slate-700/50 bg-white/70 dark:bg-slate-900/70 shadow-xl backdrop-blur-xl p-6 sm:p-8">
                                    ...
                                    <div className="flex items-center justify-between gap-4 mb-6">
                                        <div className="flex items-center gap-4">
                                            <div className="p-3 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl text-white shadow-lg shadow-indigo-500/25 opacity-80">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-slate-500 dark:text-slate-400">
                                                    <span className="font-semibold text-indigo-600 dark:text-indigo-400">{memoriesPreview.length} anı</span> kilitli • Mezuniyette açılacak
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <div className="relative overflow-hidden rounded-[1.5rem] border border-white/60 dark:border-slate-700/50 bg-white/80 dark:bg-slate-900/80 shadow-xl backdrop-blur-xl p-6 sm:p-8">
                                    ...
                                    <div className="flex items-center gap-4 mb-8 relative">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                                {isOwnProfile ? "Sınıf arkadaşlarından aldığın oylar" : "Sınıf arkadaşlarından aldığı oylar"} •{" "}
                                                <span className="font-semibold text-amber-600 dark:text-amber-400">{totalVotesForMemories} oy</span>
                                            </p>
                                        </div>
                                    </div>
                                            ...
                                            {topCategories.map((item: { category: SurveyCategory; count: number }, index: number) => (
                                                <div key={item.category.id} className="group relative">
                                                    ...
                                                    <div className={`relative overflow-hidden rounded-xl p-5 bg-gradient-to-br ${item.category.color} text-white shadow-xl`}>
                                                        <div className="absolute -right-4 -bottom-4 text-white/10 text-8xl transform group-hover:scale-110 transition-transform duration-500">
                                                            {item.category.emoji}
                                                        </div>
                                                        <div className="relative z-10">
                                                            <div className="flex items-center gap-2 mb-3">
