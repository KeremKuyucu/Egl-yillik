...
import { SURVEY_CATEGORIES, getColorFromName, getCategoryById } from "@/lib/survey-categories"
...
export default async function AdminSurveysPage() {
    ...
    const categoryResults = SURVEY_CATEGORIES.map(category => {
        const categoryVotes = allVotes?.filter(v => v.category_id === category.id) || []

        // Kişi bazında oy sayıları
        const voteCounts: Record<string, { profile: any, count: number }> = {}
        categoryVotes.forEach((vote: any) => {
            const id = vote.voted_for.id
            if (!voteCounts[id]) {
                voteCounts[id] = { profile: vote.voted_for, count: 0 }
            }
            voteCounts[id].count++
        })

        // Sırala ve ilk 5'i al
        const topVoted = Object.values(voteCounts)
    ...
    const uniqueVotedFor = new Set(allVotes?.map((v: any) => v.voted_for.id)).size
                                        ...
                                        {topVoted.map((item, index) => (
                                            ...
                                            >
                                                ...
                                                <div className="w-8 text-center">
                                                    {index === 0 && <Crown className="h-6 w-6 text-amber-500 mx-auto" />}
                                                    {index === 1 && <Medal className="h-5 w-5 text-slate-400 mx-auto" />}
                                                    {index === 2 && <Medal className="h-5 w-5 text-amber-700 mx-auto" />}
                                                    {index > 2 && <span className="text-sm font-bold text-slate-400">{index + 1}</span>}
                                                </div>

                                                {/* Avatar */}
                                                <Link href={`/profile/${item.profile.school_number}`}>
                                                    <div className={`h-10 w-10 rounded-lg flex items-center justify-center text-xs font-bold shadow-sm ${getColorFromName(item.profile.first_name)} hover:scale-105 transition-transform`}>
                                                ...
                                                {/* Oy Sayısı */}
                                                <div className="text-right">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${category.color} text-white`}>
                                                        {item.count} oy
                                                    </span>
                                                </div>
                                            </div>
