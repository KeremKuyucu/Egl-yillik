...
import type { UserWithStats, FilterStatus, FilterClass, EmailStatus } from "@/types/reminder"
import {
  BarChart,
  Bar,
  XAxis,
...
export default function ReminderClientPage({ users }: ReminderClientPageProps) {
  ...
  const stats = useMemo(() => {
    ...
    const textsIncomplete = users.filter((u) => (u.stats?.remaining_classmates ?? 0) > 0).length

    const surveysCompleted = users.filter((u) => u.surveyStats?.remaining === 0).length
    const surveysIncomplete = users.filter((u) => (u.surveyStats?.remaining ?? 0) > 0).length

    const fullyCompleted = users.filter(
      (u) => u.stats?.remaining_classmates === 0 && u.surveyStats?.remaining === 0
    ).length

    const anyIncomplete = users.filter(
      (u) => (u.stats?.remaining_classmates ?? 0) > 0 || (u.surveyStats?.remaining ?? 0) > 0
    ).length

    const byClass: Record<string, { total: number; textsComplete: number; surveysComplete: number }> = {}
    users.forEach((u) => {
      if (!byClass[u.class]) byClass[u.class] = { total: 0, textsComplete: 0, surveysComplete: 0 }
      byClass[u.class].total++
      if (u.stats?.remaining_classmates === 0) byClass[u.class].textsComplete++
      if (u.surveyStats?.remaining === 0) byClass[u.class].surveysComplete++
    })
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      ...
      if (filterClass !== "all" && user.class !== filterClass) return false
  ...
  // ✅ Seçilebilir (opt-out olmayan) filtered id listesi
  const selectableFilteredIds = useMemo(() => {
    return filteredUsers.filter((u) => !u.is_opted_out).map((u) => u.id)
  }, [filteredUsers])

  // ✅ Select-all bug fix: opt-out’ları hesaba katma
  const allFilteredSelected =
    selectableFilteredIds.length > 0 && selectableFilteredIds.every((id) => selectedUsers.includes(id))
  ...
  const toggleSelectAll = () => {
    ...
    if (allSelected) {
      setSelectedUsers((prev) => prev.filter((id) => !selectableFilteredIds.includes(id)))
    } else {
      setSelectedUsers((prev) => [...new Set([...prev, ...selectableFilteredIds])])
    }
  }

  // ✅ Guard: opted-out kullanıcı asla seçilemez
  const toggleUser = (id: string) => {
    const u = users.find((x) => x.id === id)
    if (u?.is_opted_out) return

    setSelectedUsers((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]))
  }

  // ✅ Filtrelenmiş listeden: (eksik işi var) + email var + opted-out değil
  const selectAnyIncomplete = () => {
    const incompleteIds = filteredUsers
      .filter(
        (u) =>
          ((u.stats?.remaining_classmates ?? 0) > 0 || (u.surveyStats?.remaining ?? 0) > 0) &&
          !!u.email &&
          !u.is_opted_out
      )
      .map((u) => u.id)
  ...
  const handleSendEmails = async () => {
    ...
    // ✅ ekstra güvenlik: opted-out olanlar seçili kalmışsa temizle
    const selectedSet = new Set(selectedUsers)
    const safeSelected = users
      .filter((u) => selectedSet.has(u.id) && !u.is_opted_out)
      .map((u) => u.id)

    if (safeSelected.length === 0) {
      toast.error("Seçili kullanıcıların hepsi abonelikten çıkmış görünüyor.")
      return
    }

    setIsSending(true)

    const currentResults: Record<string, EmailStatus> = { ...results }
    safeSelected.forEach((id) => (currentResults[id] = "pending"))
    setResults({ ...currentResults })

    try {
      const response = await sendBulkUsersReminders(safeSelected)

      if ((response as any)?.error) {
        toast.error((response as any).error)
        safeSelected.forEach((id) => (currentResults[id] = "error"))
        setResults({ ...currentResults })
        return
      }

      const resultsObj = (response as any)?.results as
        | Record<string, { success: boolean; error?: string; skipped?: boolean }>
        | undefined
      ...
      if (resultsObj) {
        Object.entries(resultsObj).forEach(([userId, res]) => {
      ...
      const mSent = meta?.sent ?? sent
      const mFailed = meta?.failed ?? errors
      const mSkipped = meta?.skipped_opted_out ?? 0

      if (mFailed === 0) {
        toast.success(`🎉 ${mSent} mail gönderildi${mSkipped ? ` (opt-out: ${mSkipped})` : ""}`)
      } else {
        toast.warning(`Bitti. Başarılı: ${mSent}, Hata: ${mFailed}${mSkipped ? `, Opt-out: ${mSkipped}` : ""}`)
      }
    } catch (e) {
      console.error(e)
    ...
    <div className="space-y-6">
      ...
      <div className="sticky top-20 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Kullanıcılara durum bildirimleri gönderin</p>
          </div>
        </div>
        ...
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis dataKey="name" stroke="#888888" fontSize={12} tickLine={false} axisLine={false} tick={{ fill: "#6b7280" }} />
              <YAxis stroke="#888888" fontSize={12} tickLine={false} axisLine={false} tick={{ fill: "#6b7280" }} />
              <Tooltip
              ...
              <Bar dataKey="Makale" name="Yazı Tamamlanan" fill="#10b981" radius={[4, 4, 0, 0]} barSize={20} />
              <Bar dataKey="Anket" name="Anket Tamamlanan" fill="#8b5cf6" radius={[4, 4, 0, 0]} barSize={20} />
              <Bar dataKey="Toplam" name="Toplam Öğrenci" fill="#cbd5e1" radius={[4, 4, 0, 0]} barSize={20} opacity={0.3} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <p className="text-center text-xs text-slate-400 mt-4">Detaylarını görmek veya filtrelemek için bir sınıfın sütununa tıklayın</p>
      </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              className={
                filterStatus === "texts_incomplete"
                  ? "bg-orange-500 hover:bg-orange-600 text-white border-orange-500"
                  : "text-orange-600 border-orange-300 dark:border-orange-700 hover:bg-orange-50 dark:hover:bg-orange-950/30"
              }
            >
            ...
            <Button
              ...
              className={
                filterStatus === "survey_incomplete"
                  ? "bg-purple-500 hover:bg-purple-600 text-white border-purple-500"
                  : "text-purple-600 border-purple-300 dark:border-purple-700 hover:bg-purple-50 dark:hover:bg-purple-950/30"
              }
            >
            ...
            <Button
              ...
              className={
                filterStatus === "texts_complete"
                  ? "bg-emerald-500 hover:bg-emerald-600 text-white border-emerald-500"
                  : "text-emerald-600 border-emerald-300 dark:border-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/30"
              }
            >
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Checkbox checked={allFilteredSelected && selectableFilteredIds.length > 0} onCheckedChange={toggleSelectAll} />
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">
              {filteredUsers.length} kullanıcı gösteriliyor
            </span>
          </div>
          {selectedUsers.length > 0 && (
            <Button variant="ghost" size="sm" onClick={() => setSelectedUsers([])} className="text-slate-500">
              Seçimi Temizle
            </Button>
          )}
        </div>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-[600px] overflow-y-auto">
          {filteredUsers.map((user) => {
            ...
            const textPct = clampPercent(Number(textStats?.completion_percentage ?? 0))
            ...
            return (
              <div
                ...
                className={`p-4 flex items-center gap-4 transition-colors ${
                  isSelected ? "bg-emerald-50/50 dark:bg-emerald-950/20" : "hover:bg-slate-50 dark:hover:bg-slate-800/50"
                }`}
              >
                <Checkbox checked={isSelected} onCheckedChange={() => toggleUser(user.id)} disabled={user.is_opted_out} />

                <div
                  className={`h-10 w-10 rounded-lg flex items-center justify-center text-sm font-bold shadow-sm shrink-0 ${getColorFromName(
                    user.first_name
                  )}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-slate-500 truncate">{user.email || <span className="text-red-400">Email yok</span>}</p>
                </div>
                ...
                <div className="flex flex-col items-center gap-1 min-w-[70px] sm:min-w-[80px]">
                  ...
                  {textStats && (
                    <div className="w-16 h-1 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${isTextComplete ? "bg-emerald-500" : "bg-amber-500"}`} style={{ width: `${textPct}%` }} />
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col items-center gap-1 min-w-[70px] sm:min-w-[80px]">
                  ...
                  {surveyStats && (
                    <div className="w-16 h-1 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${isSurveyComplete ? "bg-purple-500" : "bg-purple-300"}`} style={{ width: `${surveyPct}%` }} />
                    </div>
                  )}
                </div>
