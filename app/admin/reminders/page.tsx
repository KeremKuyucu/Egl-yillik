...
async function sendReminderEmail(
  ...
  const surveyProgressColor = isSurveyComplete
    ? "#8b5cf6"
    : surveyStats && surveyStats.percentage >= 50
      ? "#a855f7"
      : "#c084fc"
...
export async function sendBulkUsersReminders(userIds: string[]) {
  ...
  // Opt-out olanları skip et + sadece seçililer
  const targets = usersData.filter(
    (u) => userIds.includes(u.user_id) && !u.is_opted_out
  )
