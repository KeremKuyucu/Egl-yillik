...
export default async function ProfilePage({ params }: ProfilePageProps) {
    ...
    // Kullanıcının sınıfını al (anket sonuçları için)
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/50 to-purple-50 dark:from-slate-950 dark:via-indigo-950/50 dark:to-purple-950 text-foreground transition-colors duration-500">
            {/* Animated Background Orbs */}
            <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
                <div className="absolute top-20 -left-32 w-96 h-96 bg-gradient-to-br from-purple-400/20 to-pink-400/20 dark:from-purple-600/10 dark:to-pink-600/10 rounded-full blur-3xl animate-blob" />
                <div className="absolute top-40 -right-32 w-96 h-96 bg-gradient-to-br from-indigo-400/20 to-cyan-400/20 dark:from-indigo-600/10 dark:to-cyan-600/10 rounded-full blur-3xl animate-blob animation-delay-2000" />
                <div className="absolute -bottom-32 left-1/2 w-96 h-96 bg-gradient-to-br from-amber-400/15 to-orange-400/15 dark:from-amber-600/10 dark:to-orange-600/10 rounded-full blur-3xl animate-blob animation-delay-4000" />
            </div>

            <header className="border-b border-white/20 dark:border-slate-800/50 bg-white/70 dark:bg-slate-900/70 backdrop-blur-2xl sticky top-0 z-50 shadow-lg shadow-black/5">
                <div className="container mx-auto flex h-14 sm:h-16 items-center justify-between px-4 sm:px-6">
                    <Link href="/dashboard" prefetch={false}>
                        <Button variant="ghost" size="sm" className="gap-2 hover:bg-indigo-100/50 dark:hover:bg-indigo-900/30 transition-colors">
                            <ArrowLeft className="h-4 w-4" suppressHydrationWarning />
                            <span className="hidden sm:inline">Geri Dön</span>
                        </Button>
                    </Link>
                    <div className="flex items-center gap-2">
                        <img src="/image.png" className="h-7 w-7 sm:h-9 sm:w-9 drop-shadow-md" alt="Logo" />
                        <span className="text-lg font-bold font-serif bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-400 dark:to-purple-400 bg-clip-text text-transparent">Profil</span>
                    </div>
                    <ModeToggle />
                </div>
            </header>

            <main className="container mx-auto px-4 sm:px-6 py-6 sm:py-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
