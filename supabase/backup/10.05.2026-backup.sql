...
BEGIN
  ...
  IF add_role_level IS NULL THEN
    RAISE EXCEPTION 'Ge├ğersiz rol: %', add_role_key;
  END IF;

  -- SELF CASE: sadece daha d├╝┼ş├╝k role izin ver
  IF is_self THEN
    -- 1000 isen 999 ver, 1000/1001 verme
    IF add_role_level >= requester_max_level THEN
      RAISE EXCEPTION 'Kendinize ayn─▒ veya daha y├╝ksek bir rol atayamazs─▒n─▒z';
    END IF;
  ...
  -- hedef senden e┼şit/├╝st ise dokunma
  IF target_max_level >= requester_max_level THEN
    RAISE EXCEPTION 'Sizden daha y├╝ksek veya e┼şit yetkiye sahip bir kullan─▒c─▒n─▒n rol├╝n├╝ de─şi┼ştiremezsiniz';
  END IF;

  -- eklemek istedi─şin rol senden e┼şit/├╝st olamaz
  IF add_role_level >= requester_max_level THEN
    RAISE EXCEPTION 'Kendinizle ayn─▒ veya daha y├╝ksek bir rol atayamazs─▒n─▒z';
  END IF;
...
BEGIN
  ...
  -- Kendinden ├╝st veya e┼şit seviyedeki kullan─▒c─▒y─▒ silemez
  IF caller_level <= target_level THEN
    RAISE EXCEPTION 'Bu kullan─▒c─▒y─▒ silme yetkiniz yok';
  END IF;

  -- Zaten silinmi┼ş mi kontrol et
  IF EXISTS (
    SELECT 1 FROM public.profiles
    WHERE id = target_user_id AND deleted_at IS NOT NULL
  ) THEN
    RAISE EXCEPTION 'Bu hesap zaten silinmi┼ş';
  END IF;
...
BEGIN
  ...
  IF target_user_id = auth.uid() THEN
    RAISE EXCEPTION 'Kendi rol├╝n├╝z├╝ de─şi┼ştiremezsiniz';
  END IF;
  ...
  IF remove_role_level IS NULL THEN
    RAISE EXCEPTION 'Ge├ğersiz rol: %', remove_role_key;
  END IF;
  ...
  -- kald─▒ramayaca─ş─▒n rol: senden e┼şit/├╝st seviyede
  IF remove_role_level >= requester_max_level THEN
    RAISE EXCEPTION 'Kendinizle ayn─▒ veya daha y├╝ksek bir rol├╝ kald─▒ramazs─▒n─▒z';
  END IF;
...
DECLARE
  ...
  target_level integer;
BEGIN
  -- ─░zin kontrol├╝ (silme ile ayn─▒ izin)
  PERFORM public.require_permission('admin.account.delete');
  ...
  -- Kendinden ├╝st veya e┼şit seviyedeki kullan─▒c─▒y─▒ geri getiremez
  IF caller_level <= target_level THEN
    RAISE EXCEPTION 'Bu kullan─▒c─▒y─▒ geri getirme yetkiniz yok';
  END IF;

  -- Silinmemi┼ş hesap kontrol├╝
  IF NOT EXISTS (
    SELECT 1 FROM public.profiles
    WHERE id = target_user_id AND deleted_at IS NOT NULL
  ) THEN
    RAISE EXCEPTION 'Bu hesap silinmemi┼ş';
  END IF;
...
BEGIN
  ...
  -- ─░ste─şe ba─şl─▒ ama g├╝├ğl├╝ g├╝venlik
...
perform public.require_permission('admin.users.update');
  ...
  IF caller_level <= target_level THEN
    RETURN jsonb_build_object('success', false, 'error', 'Bu kullan─▒c─▒y─▒ d├╝zenleme yetkiniz yok');
  END IF;
  ...
  IF v_rows = 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'Profil g├╝ncellenemedi');
  END IF;
...
DECLARE
    reg_status TEXT;
BEGIN
    -- site_settings tablosundan registration_enabled anahtar─▒n─▒n de─şerini ├ğek
    SELECT value INTO reg_status 
    FROM public.site_settings 
    WHERE key = 'registration_enabled';

    -- E─şer kay─▒t "true" de─şilse i┼şlemi engelle ve hata d├Ând├╝r
    -- (null olma ihtimaline kar┼ş─▒ coalesce veya do─şrudan kontrol)
    IF reg_status IS NULL OR reg_status <> 'true' THEN
        RAISE EXCEPTION 'Yeni kay─▒tlar ┼şu anda kapal─▒d─▒r.' 
        USING ERRCODE = 'P0001',
              DETAIL = 'site_settings tablosundaki registration_enabled anahtar─▒ true de─şil.';
    END IF;
...
BEGIN
  ...
  -- Sadece y─▒l bazl─▒ deadline (global yok)
  ...
  -- S├╝re ge├ğtiyse kilitle
  IF now() > v_deadline THEN
    RAISE EXCEPTION 'Mezuniyet tarihi (%) ge├ğti─şi i├ğin i┼şlem yap─▒lamaz.', v_deadline;
  END IF;
...
BEGIN
    ------------------------------------------------------------------
    -- SELF VOTE L─░M─░T─░ (farkl─▒ kategorilerde max 3)
    ------------------------------------------------------------------
    IF NEW.voted_for_id IS NOT NULL
       AND NEW.voter_id = NEW.voted_for_id THEN
        ...
        WHERE voter_id = NEW.voter_id
          ...
          -- UPDATE senaryosunda kendini saymamas─▒ i├ğin
          AND (TG_OP = 'INSERT' OR id <> OLD.id);

        IF self_vote_count >= 3 THEN
            RAISE EXCEPTION
                'En fazla 3 farkl─▒ kategoride kendinize oy verebilirsiniz.';
        END IF;
    END IF;

    ------------------------------------------------------------------
    -- Manuel giri┼ş (custom option)
    ------------------------------------------------------------------
    ...
    IF voter_info IS NULL OR target_info IS NULL THEN
        RAISE EXCEPTION 'Profil bilgisi bulunamad─▒.';
    END IF;
    ...
    IF voter_info.user_year <> target_info.user_year THEN
        RAISE EXCEPTION
            'Farkl─▒ mezuniyet y─▒l─▒ndaki (Nesil: %) kullan─▒c─▒ya oy verilemez.',
    ...
    IF voter_info.class <> target_info.class THEN
        RAISE EXCEPTION
            'Sadece kendi s─▒n─▒f─▒n─▒zdaki (%) ki┼şilere oy verebilirsiniz.',
            voter_info.class;
    END IF;
...
BEGIN
  ...
  IF v_uid IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Oturum bulunamad─▒');
  END IF;

  -- h─▒zl─▒ bo┼ş kontrol (as─▒l validasyon trigger'da)
  ...
  IF COALESCE(btrim(p_school_number), '') = '' THEN
    RETURN jsonb_build_object('success', false, 'error', 'Okul numaras─▒ gerekli');
  END IF;

  IF COALESCE(btrim(p_class_room), '') = '' THEN
    RETURN jsonb_build_object('success', false, 'error', 'S─▒n─▒f gerekli');
  END IF;

  -- profil zaten var m─▒?
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = v_uid AND deleted_at IS NULL) THEN
    RETURN jsonb_build_object('success', false, 'error', 'Profil zaten olu┼şturulmu┼ş.');
  END IF;
  ...
  EXCEPTION
    WHEN unique_violation THEN
      ...
      IF v_constraint = 'profiles_school_number_year_unique' THEN
        RETURN jsonb_build_object('success', false, 'error', 'Bu okul numaras─▒ bu d├Ânem i├ğin zaten al─▒nm─▒┼ş.');
      ELSIF v_constraint = 'profiles_pkey' THEN
        RETURN jsonb_build_object('success', false, 'error', 'Profil zaten olu┼şturulmu┼ş.');
      ELSE
        RETURN jsonb_build_object('success', false, 'error', 'Bu kay─▒t zaten mevcut.');
      END IF;

    WHEN foreign_key_violation THEN
      RETURN jsonb_build_object('success', false, 'error', 'Kullan─▒c─▒ kayd─▒ bulunamad─▒.');
...
BEGIN
  ...
  -- Email opt-out (silince otomatik abonelikten ├ğ─▒kar)
  INSERT INTO public.email_opt_outs (user_id)
  VALUES (v_uid)
  ON CONFLICT (user_id)
  DO UPDATE SET updated_at = now();
...
BEGIN
  ...
  IF author_year IS NULL THEN
    RAISE EXCEPTION 'author_id i├ğin profil yok veya user_year NULL';
  END IF;
  ...
  IF recipient_year IS NULL THEN
    RAISE EXCEPTION 'recipient_id i├ğin profil yok veya user_year NULL';
  END IF;

  -- same year check
  IF author_year IS DISTINCT FROM recipient_year THEN
    RAISE EXCEPTION 'Farkl─▒ y─▒llar aras─▒nda mesaj g├Ânderilemez (author_year=%, recipient_year=%)',
...
CREATE OR REPLACE FUNCTION "public"."get_activity_logs_latest"("p_limit" integer DEFAULT 100) RETURNS TABLE("id" "uuid", "table_name" "text", "operation" "text", "record_id" "text", "old_data" "jsonb", "new_data" "jsonb", "changed_at" timestamp with time zone, "changed_by" "uuid", "profile_first_name" "text", "profile_last_name" "text", "profile_class" "text")
    ...
    AS $$
BEGIN
  -- Yetki kontrol├╝ (auth + permission i├ğeride)
...
BEGIN
    -- Yetki Kontrol├╝ (Min Level 100 - Super Admin)
    perform public.require_permission('admin.votes.read');

    -- LOGLAMA: Eri┼şimi kaydet
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Kategori bulunamad─▒';
    END IF;
...
perform public.require_permission('admin.stats.read');
    ...
    -- Feedback Say─▒s─▒ (Tablo varsa ve silinmemi┼ş olanlar)
...
BEGIN
  ...
  -- ÔöÇÔöÇÔöÇ 1. Global stats (hafif, profile join yok) ÔöÇÔöÇÔöÇ
  SELECT
    count(*),
  ...
  -- ÔöÇÔöÇÔöÇ 2. Distinct s─▒n─▒f listesi ÔöÇÔöÇÔöÇ
  ...
  -- ÔöÇÔöÇÔöÇ 3. Distinct y─▒llar ÔöÇÔöÇÔöÇ
  ...
  -- ÔöÇÔöÇÔöÇ 4. Filtrelenmi┼ş total + sayfalanm─▒┼ş items (TEK WITH BLO─ŞU) ÔöÇÔöÇÔöÇ
  WITH unified AS (
    SELECT
      t.id::text           AS id,
      t.created_at,
...
CREATE OR REPLACE FUNCTION "public"."get_admin_vote_access_logs"("p_limit" integer DEFAULT 100) RETURNS TABLE("id" "uuid", "admin_id" "uuid", "category_id" "text", "accessed_at" timestamp with time zone, "admin" json, "category_info" json)
    ...
    AS $$
BEGIN
    -- Yetki Kontrol├╝: system.votes.access_log izni gerekli
...
BEGIN
    ...
    -- Profil var m─▒ + soft delete kontrol
    ...
    -- last_active'─▒ log tablosundan ├ğek (en son event)
    ...
    -- Settings (deadline + sadece o y─▒l─▒n graduation_date)
    SELECT
    ...
    SELECT
        COUNT(*)::int,
        COALESCE(SUM(regexp_count(t.content, '[[:alpha:]]+(?:[''ÔÇÖ.-][[:alpha:]]+)*'))::int, 0),
        MAX(t.updated_at)
...
BEGIN
    ...
    FROM (
        SELECT jsonb_build_object(
            ...
            'caption', gp.caption,
            'created_at', gp.created_at,
            'is_unlocked', true, -- Kendi foto─şraf─▒ oldu─şu i├ğin varsay─▒lan true
            'profiles', jsonb_build_object(
                'first_name', p.first_name,
                'last_name', p.last_name,
                'school_number', p.school_number,
...
BEGIN
  ...
  IF p_bucket NOT IN ('hour', 'day', 'week') THEN
    RAISE EXCEPTION 'Ge├ğersiz bucket: %. hour|day|week olmal─▒', p_bucket;
  END IF;
...
BEGIN
    -- 1) Profil + say─▒mlar (view ├╝zerinden)
    SELECT *
    INTO v_profile
    ...
    -- Photos Preview (kilitliyken sadece yap─▒sal veri)
    SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
            'id', gp.id,
            'created_at', gp.created_at
...
COMMENT ON FUNCTION "public"."get_profile_page_extended_data"("target_school_number" "text", "target_year" smallint) IS 'Profil sayfas─▒ i├ğin geni┼şletilmi┼ş veri getiren fonksiyon.
- Sadece service_role eri┼şebilir (server-side calls only)
- Normal kullan─▒c─▒lar bu fonksiyonu do─şrudan ├ğa─ş─▒ramaz';
...
BEGIN
  ...
  IF v_owner IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Oturum a├ğ─▒lmam─▒┼ş.');
  END IF;
  ...
  IF NOT v_messaging THEN
    RETURN jsonb_build_object('success', false, 'error', 'Mesaj yazma ┼şu anda kapal─▒d─▒r.');
  END IF;

  -- 3) Kendine yazma engeli
  IF v_owner = p_recipient_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'Kendinize anonim mesaj yazamazs─▒n─▒z.');
  END IF;

  -- 4) Al─▒c─▒ var m─▒?
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = p_recipient_id) THEN
    RETURN jsonb_build_object('success', false, 'error', 'Al─▒c─▒ bulunamad─▒.');
  END IF;

  -- 5) display_name: bo┼şsa 'Anonim'
  v_name := COALESCE(NULLIF(btrim(p_display_name), ''), 'Anonim');

  -- 5.1) content trim + bo┼ş kontrol (CHECK constraint var ama hata mesaj─▒n─▒ d├╝zg├╝n d├Ând├╝relim)
  v_content := btrim(COALESCE(p_content, ''));
  IF char_length(v_content) < 1 THEN
    RETURN jsonb_build_object('success', false, 'error', 'Mesaj bo┼ş olamaz.');
  END IF;

  -- 6) Ayn─▒ ki┼şi ├ğiftinde eski aktifleri kapat (varsa)
...
EXCEPTION
  WHEN unique_violation THEN
    -- Bu art─▒k normalde olmamal─▒; olursa concurrency/race vard─▒r
    RETURN jsonb_build_object('success', false, 'error', '─░┼şlem ├ğak─▒┼şt─▒, tekrar deneyin.');
  WHEN OTHERS THEN
    RETURN jsonb_build_object('success', false, 'error', SQLERRM);
END;
$$;
...
BEGIN
  ...
  -- PK kolonlar─▒n─▒ bul (public schema varsay─▒m─▒)
...
BEGIN
    ...
    IF v_author_id IS NULL THEN
        RETURN jsonb_build_object('success', false, 'error', 'Metin bulunamad─▒.');
    END IF;

    -- ÔØù messaging kapal─▒ysa: sadece admin silebilsin (eski davran─▒┼ş─▒ korur)
    IF NOT v_messaging_enabled THEN
        RETURN jsonb_build_object('success', false, 'error', 'Mesajla┼şma ┼şu anda kapal─▒.');
    END IF;
    ...
    IF v_user_id = v_author_id OR v_is_admin THEN
        ...
        RETURN jsonb_build_object('success', true, 'message', 'Metin ba┼şar─▒yla pasife ├ğekildi.');
    END IF;

    RETURN jsonb_build_object('success', false, 'error', 'Bu i┼şlem i├ğin yetkiniz yok.');
  ...
  if tg_op = 'INSERT' then
    ...
    if v_new_active then
      v_new_words := coalesce(
        regexp_count(NEW.content, '[[:alpha:]]+(?:[''ÔÇÖ.-][[:alpha:]]+)*'),
        0
      );
    ...
    if v_old_active then
      v_old_words := coalesce(
        regexp_count(OLD.content, '[[:alpha:]]+(?:[''ÔÇÖ.-][[:alpha:]]+)*'),
        0
      );
...
BEGIN
    ...
    ----------------------------------------------------------------
    -- AYAR KONTROLLER─░ (ZORUNLU)
    ----------------------------------------------------------------
    IF active_year IS NULL THEN
        RAISE EXCEPTION 'active_year ayar─▒ bulunamad─▒';
    END IF;

    IF valid_classes IS NULL THEN
        RAISE EXCEPTION 'valid_classes ayar─▒ bulunamad─▒';
    END IF;

    IF school_number_len IS NULL THEN
        RAISE EXCEPTION 'school_number_length ayar─▒ bulunamad─▒';
    END IF;

    IF school_number_pattern IS NULL THEN
        RAISE EXCEPTION 'school_number_regex ayar─▒ bulunamad─▒';
    END IF;
    ...
    IF TG_OP = 'INSERT' THEN
        -- Frontend ne g├Ânderirse g├Ândersin YOK SAY
    NEW.user_year := active_year;

    ELSIF TG_OP = 'UPDATE' THEN
        -- De─şi┼ştirmeye ├ğal─▒┼ş─▒yorsa ENGELLE
        IF NEW.user_year IS DISTINCT FROM OLD.user_year THEN
            RAISE EXCEPTION 'user_year de─şi┼ştirilemez';
        END IF;
    END IF;
    ...
    IF NEW.class IS NULL OR NEW.class = '' THEN
        RAISE EXCEPTION 'class bo┼ş olamaz';
    END IF;

    IF NOT (NEW.class = ANY(valid_classes)) THEN
        RAISE EXCEPTION 'Ge├ğersiz class: %', NEW.class;
    END IF;
    ...
    IF NEW.school_number IS NULL OR NEW.school_number = '' THEN
        RAISE EXCEPTION 'school_number bo┼ş olamaz';
    END IF;

    IF length(NEW.school_number) != school_number_len THEN
        RAISE EXCEPTION 'Ge├ğersiz school_number uzunlu─şu: %', NEW.school_number;
    END IF;

    IF NEW.school_number !~ school_number_pattern THEN
        RAISE EXCEPTION 'Ge├ğersiz school_number format─▒: %', NEW.school_number;
    END IF;
...
BEGIN
    ...
    IF v_content IS NULL THEN
        RETURN jsonb_build_object('success', false, 'error', 'Anonim yaz─▒ bulunamad─▒.');
    END IF;
...
BEGIN
    ...
    IF v_content IS NULL THEN
        RETURN jsonb_build_object('success', false, 'error', 'Yaz─▒ bulunamad─▒.');
    END IF;
...
COMMENT ON TABLE "public"."feedback" IS 'Kullan─▒c─▒ geri bildirimleri';
...
CREATE POLICY "Profilleri herkes g├Ârebilir" ON "public"."profiles" FOR SELECT USING (("deleted_at" IS NULL));



CREATE POLICY "Rolleri herkes g├Ârebilir" ON "public"."roles" FOR SELECT USING (true);
