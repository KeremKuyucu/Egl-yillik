...
CREATE OR REPLACE FUNCTION "public"."admin_update_user_role"("target_user_id" "uuid", "new_role_key" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_catalog'
    AS $$
DECLARE
...
BEGIN
  ...
  INTO new_role_level
  FROM public.roles r
  WHERE r.key = new_role_key;

  IF new_role_level IS NULL THEN
    RAISE EXCEPTION 'Geçersiz rol: %', new_role_key;
  END IF;

  -- (Opsiyonel) Owner ataması sadece owner'a
  IF new_role_key = 'owner' AND requester_max_level < 1000 THEN
    RAISE EXCEPTION 'Owner rolünü sadece Owner atayabilir';
  END IF;

  -- Kural 2: kendinle aynı/üst role atayamazsın
  IF new_role_level >= requester_max_level THEN
    RAISE EXCEPTION 'Kendinizle aynı veya daha yüksek bir rol atayamazsınız';
  END IF;

  -- Kural 3: senden eşit/üst kişiyi düzenleyemezsin
  ...
  -- Rolleri REPLACE et (tek rol sistemi)
  DELETE FROM public.user_roles
  WHERE user_id = target_user_id;

  INSERT INTO public.user_roles (user_id, role_key, created_at)
  VALUES (target_user_id, new_role_key, now())
...
ALTER FUNCTION "public"."admin_update_user_role"("target_user_id" "uuid", "new_role_key" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."create_user_level_row"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'pg_catalog'
    AS $$
BEGIN
  INSERT INTO public.user_levels (id, level, source, set_by, set_at)
  VALUES (NEW.id, 1, 'profiles_insert', NULL, now())
...
ALTER FUNCTION "public"."create_user_level_row"() OWNER TO "postgres";
...
BEGIN
  ...
  survey_stats AS (
    SELECT sv.voter_id AS user_id, COUNT(DISTINCT sv.category_id)::integer AS completed
...
CREATE OR REPLACE FUNCTION "public"."get_my_permissions"() RETURNS "text"[]
    ...
    AS $$
  WITH my AS (
    SELECT COALESCE(MAX(r.level), 0) AS my_level
    FROM public.user_roles ur
    JOIN public.roles r ON r.key = ur.role_key
    WHERE ur.user_id = auth.uid()
  )
  ...
  WHERE r.level <= (SELECT my_level FROM my);
$$;
...
CREATE OR REPLACE FUNCTION "public"."has_permission"("p_perm" "text") RETURNS boolean
  ...
  required_level AS (
    SELECT COALESCE(MIN(r.level), 999999) AS lvl
    FROM public.role_permissions rp
    JOIN public.roles r ON r.key = rp.role_key
    WHERE rp.perm_key = p_perm
  )
  SELECT (SELECT lvl FROM my_level) >= (SELECT lvl FROM required_level);
$$;
...
CREATE OR REPLACE VIEW "public"."texts_memories_v" WITH ("security_invoker"='on') AS
 SELECT "r"."id" AS "recipient_id",
    ...
    "r"."class",
    "r"."user_year",
    COALESCE("json_agg"("json_build_object"('first_name', "a"."first_name", 'last_name', "a"."last_name", 'school_number', "a"."school_number", 'class', "a"."class", 'user_year', "a"."user_year", 'content', "t"."content", 'created_at', "t"."created_at", 'id', "t"."id", 'author_id', "t"."author_id") ORDER BY "t"."created_at" DESC) FILTER (WHERE ("t"."id" IS NOT NULL)), '[]'::json) AS "memories"
   FROM (("public"."profiles" "r"
     LEFT JOIN "public"."texts" "t" ON ((("t"."recipient_id" = "r"."id") AND ("t"."is_active" IS TRUE) AND ("t"."author_id" <> "t"."recipient_id"))))
     LEFT JOIN "public"."profiles" "a" ON (("a"."id" = "t"."author_id")))
  GROUP BY "r"."id", "r"."first_name", "r"."last_name", "r"."school_number", "r"."class", "r"."user_year";
...
CREATE OR REPLACE TRIGGER "trg_create_user_level_row" AFTER INSERT ON "public"."profiles" FOR EACH ROW EXECUTE FUNCTION "public"."create_user_level_row"();
...
CREATE POLICY "admin_can_manage_roles" ON "public"."user_roles" USING ("public"."has_permission"('admin.roles.update'::"text")) WITH CHECK ("public"."has_permission"('admin.roles.update'::"text"));
...
CREATE POLICY "user_can_see_own_roles" ON "public"."user_roles" FOR SELECT USING (("user_id" = "auth"."uid"()));
...
GRANT ALL ON FUNCTION "public"."admin_update_user_role"("target_user_id" "uuid", "new_role_key" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."admin_update_user_role"("target_user_id" "uuid", "new_role_key" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."admin_update_user_role"("target_user_id" "uuid", "new_role_key" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_user_level_row"() TO "anon";
GRANT ALL ON FUNCTION "public"."create_user_level_row"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_user_level_row"() TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."get_bulk_user_stats"("user_ids" "uuid"[]) FROM PUBLIC;
