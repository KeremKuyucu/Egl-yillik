-- get_admin_users_list: last_active artık profile_last_active_log tablosundan çekiliyor
-- profiles tablosunda last_active kolonu kaldırıldığı için güncellendi.

CREATE OR REPLACE FUNCTION public.get_admin_users_list(class_filter text DEFAULT NULL::text, search_query text DEFAULT NULL::text, sort_by text DEFAULT 'role'::text)
 RETURNS TABLE(id uuid, first_name text, last_name text, school_number text, class text, user_year smallint, last_active timestamp with time zone, role_level integer, highest_role_key text, is_deleted boolean, deleted_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
