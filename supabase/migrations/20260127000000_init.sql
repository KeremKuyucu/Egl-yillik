-- =====================================================
-- EGL YILLIK - INITIAL CONSOLIDATED MIGRATION (27.01.2026)
-- Based on optimized backup
-- =====================================================
...
BEGIN
    ...
    -- get_my_level() fonksiyonu user_levels tablosuna bakar
...
CREATE OR REPLACE FUNCTION public.get_admin_users_list(
    ...
    sort_by text DEFAULT 'level'
)
RETURNS TABLE(id uuid, first_name text, last_name text, school_number text, class text, last_active timestamp with time zone, level integer)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_catalog'
AS $function$
BEGIN
    ...
    ORDER BY 
        ...
        CASE WHEN sort_by = 'level' THEN COALESCE(ul.level, 0) END DESC,
        p.last_name ASC;
END;
$function$;
